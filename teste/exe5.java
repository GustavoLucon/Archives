...
class Arvore{
	public Bintn raiz;
	public Bintn inserir (Bintn ar, int novonum){
		if(ar == null){
			return new Bintn(novonum);
		}
		else{
			if(novonum<ar.valor){
...
private void exibiresquerda(Bintn arv){
	if(arv !=null){
		exibiresquerda(arv.esq);
		System.out.println(arv.valor+ " ");
	}
}
private void exibirdireita(Bintn arv){
	if(arv != null){
