...
public class Arena extends Activity {
	...
	private void setupAgarra() {
		// TODO Auto-generated method stub
		Button Botaochampselect = (Button) findViewById(R.id.Agarrar);
		// Botaochampselect.setBackgroundColor(Color.TRANSPARENT);
		Botaochampselect.setOnClickListener(new View.OnClickListener() {
	...
	protected void Combater() {
		...
		for (int i = 0; i < 3; i++) {
			if (derrotado == false) {
				if (listaGolpes[i] == 0) {
					if (player.getEspecial() == 0 && especial == true) {
						if (resposta[i] == 0) {
							resultado = resultado
									+ "Round:"
									+ (i + 1)
					...
					if (player.getEspecial() == 1 && especial == true) {
						if (resposta[i] == 0) {
							resultado = resultado
									+ "Round:"
									+ (i + 1)
					...
					if (player.getEspecial() == 2 && especial == true) {
						if (resposta[i] == 0) {
							resultado = resultado
									+ "Round:"
									+ (i + 1)
	...
	private void Derrotado(int i) {
		...
		Button Btnagr = (Button) findViewById(R.id.Agarrar);
	...
	private void TravarBotoes(int travar) {
		...
		Button Btnatq = (Button) findViewById(R.id.Ataque);
		Button Btndef = (Button) findViewById(R.id.Defend);
		Button Btnesp = (Button) findViewById(R.id.Agarrar);
		if (travar == 0) {
			Btnatq.setEnabled(false);
			Btndef.setEnabled(false);
			Btnesp.setEnabled(false);
	...
	private boolean ConfereDerrota(Luchadores personagem) {
		// TODO Auto-generated method stub
		if (personagem.getVida() == 0 || personagem.getVida() < 0) {
			return true;
		} else {
			return false;
		}
